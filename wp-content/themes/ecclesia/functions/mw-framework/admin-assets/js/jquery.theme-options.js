  ...
  inlineEditOption = {
    init: function () {
      ...
      $('.reset', '#the-theme-options').live("click", function () {
        var agree = confirm("Are you sure you wish to reset all theme options to default settings?");
        if (agree) {
    ...
    save_options: function (e) {
      var d = {
        action: "mw_save_theme_options"
      };
      b = $(':input', '#the-theme-options').serialize();
      d = b + "&" + $.param(d);
      $.post(ajaxurl, d, function (r) {
        if (r != -1) {
          $('.ajax-message').ajaxMessage('<div class="message"><span>&nbsp;</span>Theme options saved</div>');
    ...
    reset_options: function () {
      var d = {
        action: "mw_reset_theme_options",
        _ajax_nonce: $("#_ajax_nonce").val()
      };
      $.post(ajaxurl, d, function (r) {
        if (r != -1) {
          alert('Theme options have been reset to default settings');
          location.reload();
        } else {
          $('.ajax-message').ajaxMessage('<div class="message warning"><span>&nbsp;</span>Theme options could not be reset</div>');
        }
      });
      return false;
    },
    import_data: function (e) {
      var d = {
        action: "mw_import_theme_options"
      };
